...
Options:
  ...
  --venv DIR            Virtual environment directory inside this repository. Default: .venv
...
venv_path="$("$python_cmd" -c 'import os, sys; print(os.path.realpath(sys.argv[1]))' "$venv_dir")"
case "$venv_path" in
  "$repo_root"/*) ;;
  *)
    echo "Refusing virtual environment outside repository: $venv_path" >&2
    exit 2
    ;;
esac
