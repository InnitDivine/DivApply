...
def resolve_all_urls(conn: sqlite3.Connection) -> dict:
    ...
    rows = conn.execute("SELECT url, site FROM jobs WHERE archived_at IS NULL").fetchall()
    resolved = 0
    failed = 0
    already_absolute = 0
...
def resolve_wttj_urls(conn: sqlite3.Connection) -> int:
    ...
    wttj_jobs = conn.execute(
        "SELECT url, title FROM jobs WHERE site = 'WelcomeToTheJungle' AND archived_at IS NULL"
    ).fetchall()
...
def _run_detail_scraper(
    ...
    where = f"WHERE detail_scraped_at IS NULL AND archived_at IS NULL AND {skip_filter}"
    rows = conn.execute(
    ...
    try:
        while True:
            ...
            rows = conn.execute(
                ...
                f"WHERE detail_scraped_at IS NULL AND archived_at IS NULL AND {skip_filter} "
