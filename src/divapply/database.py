...
def get_connection(db_path: Path | str | None = None) -> sqlite3.Connection:
    ...
    for private_path in (
        ...
        if private_path.exists():
            protect_file(private_path, strict=True)
    ...
    Schema columns by stage:
      ...
      - Scoring:    fit_score, llm_score, keyword_score, embedding_score,
                   ...
                   matched_skills, missing_skills, keyword_hits, risk_flags,
                    apply_or_skip_reason, scored_at, score_attempts,
                    score_error, score_retry_at
...
def ensure_job_indexes(conn: sqlite3.Connection | None = None) -> None:
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_jobs_score_retry ON jobs(fit_score, score_attempts, score_retry_at)")
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_jobs_dashboard_site ON jobs(site) WHERE archived_at IS NULL")
    conn.execute(
...
def delete_scored_jobs_at_or_below(
    ...
    *,
    ...
    score_predicate = "fit_score IS NOT NULL AND fit_score <= ? AND archived_at IS NULL"
    if positive_only:
...
def record_reliability_event(
    ...
    *,
    ...
    try:
        ...
        if should_commit and conn is not None and conn.in_transaction:
            conn.rollback()
...
def add_application_event(
    ...
    *,
    ...
    if event_id is None:
        raise RuntimeError("SQLite did not return an application event row id")
    return event_id
...
def _infer_course_skills(entry: dict) -> list[str]:
    ...
    rules: list[tuple[tuple[str, ...], list[str]]] = [
        ...
        (
            ("business communications", "business english", "composition", "technical writing", "writing"),
            ["professional writing", "documentation", "communication"],
        ),
        (
            ("information systems", "computer", "information technology", "is 101"),
            ["information systems", "digital workflows", "business software"],
        ),
        (("marketing",), ["marketing", "customer awareness", "communication"]),
        (
            ("management", "operations", "entrepreneurship", "legal environment", "personal finance"),
            ["business operations", "management", "compliance", "planning"],
        ),
        (
            (
                "public health",
            ...
            ["public health", "health education", "community services", "program support"],
        ),
        (
            ("nutrition", "medical terminology", "anatomy", "physiology", "disability"),
            ["health literacy", "human services", "wellness"],
        ),
        ...
        (
            ("psychology", "human diversity", "history", "culture", "arts", "music", "new testament"),
            ["interpersonal communication", "cultural awareness", "behavioral insight"],
        ),
...
def append_coursework(entries: list[dict], conn: sqlite3.Connection | None = None) -> dict:
    ...
    for row in conn.execute("SELECT school, course_code, course_title, term FROM coursework").fetchall():
...
def get_education_records(conn: sqlite3.Connection | None = None) -> list[dict]:
    """Return canonical school/program facts from an optional transcript import.

    Structured transcript tables are user-imported extensions, so databases
    created by DivApply alone legitimately return an empty list.
    """
    if conn is None:
        conn = get_connection()
    ...
    program_columns = """
        NULL AS degree, NULL AS major, NULL AS minor, NULL AS degree_status,
        NULL AS confer_date, NULL AS expected_graduation_year, NULL AS delivery
    """
    join_clause = ""
    if "education_programs" in table_names:
        program_columns = """
            p.degree, p.major, p.minor, p.degree_status, p.confer_date,
            p.expected_graduation_year, p.delivery
        """
        join_clause = """
            LEFT JOIN education_programs p ON p.id = (
                SELECT MIN(ep.id) FROM education_programs ep
                WHERE ep.school_id = s.school_id
            )
        """

    rows = conn.execute(
        f"""
        SELECT s.school_id, s.name, s.city_state, s.start_year, s.end_year,
               s.attendance_status, s.institutional_gpa, s.program_gpa,
               s.gpa_as_of, s.institutional_earned, s.total_earned,
               s.updated_at, {program_columns}
        FROM schools s
        {join_clause}
        ORDER BY s.start_year DESC, s.name
        """
    ).fetchall()
    return [dict(row) for row in rows]
...
def get_stats(conn: sqlite3.Connection | None = None) -> dict:
    ...
    stats["archived"] = int(conn.execute("SELECT COUNT(*) FROM jobs WHERE archived_at IS NOT NULL").fetchone()[0])

    # By site breakdown
    rows = conn.execute(
        "SELECT site, COUNT(*) as cnt FROM jobs WHERE archived_at IS NULL GROUP BY site ORDER BY cnt DESC"
    ).fetchall()
    ...
    try:
        ensure_reliability_events_table(conn)
        stats["reliability_errors"] = int(
            conn.execute("SELECT COUNT(*) FROM reliability_events WHERE severity IN ('error', 'critical')").fetchone()[
                0
            ]
        )
    except Exception:
...
def archive_job(
    ...
    *,
    ...
    with _transaction(conn):
        row = conn.execute(
            "SELECT tailored_resume_path, cover_letter_path, apply_status "
            "FROM jobs WHERE url = ? AND archived_at IS NULL",
            (url,),
        ).fetchone()
        if row is None:
            return False
        if row["apply_status"] == "in_progress":
            return False
        from divapply.artifacts import shared_job_artifact_paths

        protected_paths = shared_job_artifact_paths({"url": url, **dict(row)}, conn=conn)
        cursor = conn.execute(
            """
            UPDATE jobs
            SET archived_at = ?,
    ...
    delete_job_artifacts(dict(row), protected_paths=protected_paths)
...
_STAGE_CONDITIONS = {
    ...
    "pending_score": (
        f"{MEANINGFUL_FULL_DESCRIPTION_SQL} AND fit_score IS NULL "
        "AND COALESCE(score_attempts, 0) < 5 "
        "AND (score_retry_at IS NULL OR datetime(score_retry_at) <= datetime('now'))"
    ),
...
def _stage_where(stage: str, min_score: int | None) -> tuple[str, list[Any]]:
    """Build one stage predicate/parameter set for selection and counting."""
    try:
    ...
    params: list[Any] = []
    if "?" in where:
        params.append(7 if min_score is None else min_score)
...
def count_jobs_by_stage(
    ...
    """Count jobs using the exact predicate owned by ``get_jobs_by_stage``."""
    if conn is None:
        conn = get_connection()
    where, params = _stage_where(stage, min_score)
