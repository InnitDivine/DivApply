...
from divapply.security import copy_private_file, write_private_text
...
def _setup_resume() -> None:
    ...
    while True:
        ...
        if suffix == ".txt":
            copy_private_file(src, RESUME_PATH)
            console.print(f"[green]Copied to {RESUME_PATH}[/green]")
        elif suffix == ".pdf":
            copy_private_file(src, RESUME_PDF_PATH)
            ...
            if txt_path_str.strip():
                ...
                if txt_src.exists():
                    copy_private_file(txt_src, RESUME_PATH)
                    console.print(f"[green]Copied to {RESUME_PATH}[/green]")
                else:
...
def _setup_profile() -> dict:
    ...
    profile["work_authorization"] = {
        ...
        "work_permit_type": Prompt.ask(
            "Work permit type (e.g. Citizen, PR, Open Work Permit â€” leave blank if N/A)", default=""
        ),
    }
...
def _setup_searches() -> None:
    ...
    write_private_text(SEARCH_CONFIG_PATH, "\n".join(lines) + "\n")
...
def _setup_auto_apply() -> None:
    ...
    console.print(
        "[dim]CAPTCHAs stop an auto-apply run safely; DivApply does not request "
        "solver credentials or expand agent permissions.[/dim]"
    )
