...
def generate_dashboard(
    ...
    *,
    ...
    for score_value in range(10, 0, -1):
        count = score_dist.get(score_value, 0)
        pct = (count / max_count * 100) if max_count else 0
        score_color = _score_color(score_value)
        score_bar_parts.append(f"""
        <div class="score-row" aria-label="Score {score_value}: {count} jobs">
          <span class="score-label">{score_value}</span>
    ...
    for site_stat in site_stats:
        ...
        high_pct = site_stat["high_fit"] / max(site_stat["total"], 1) * 100
        mid_pct = site_stat["mid_fit"] / max(site_stat["total"], 1) * 100
        ...
        <div class="site-row">
          ...
          <div class="bar-track" aria-label="{escape(site)}: {site_stat['high_fit']} strong fit and {site_stat['mid_fit']} moderate fit jobs">
