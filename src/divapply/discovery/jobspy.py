...
def parse_proxy(proxy_str: str) -> dict:
    ...
    if len(parts) == 4:
        ...
        raise ValueError("Proxy format not recognized. Expected: host:port:user:pass or host:port")
...
def _row_is_effectively_remote(row) -> bool:
    ...
    location_has_remote_tag = any(token in location for token in REMOTE_TERMS)
    concrete_location = re.sub(
        r"\b(?:remote|hybrid|work from home|wfh)\b|[(),\-]",
        " ",
        location,
    )
    concrete_location = re.sub(r"\s+", " ", concrete_location).strip()
    broad_locations = {
        "", "in", "us", "usa", "united states", "united states of america", "nationwide",
    }
    has_concrete_location = concrete_location not in broad_locations
    if location_has_remote_tag and not has_concrete_location:
        return True

    evidence = " ".join(
        str(row.get(key, "") or "") for key in ("title", "company", "description")
    ).lower()
    if any(phrase in evidence for phrase in _REMOTE_EVIDENCE_PHRASES):
        return True

    broad_location = not has_concrete_location
    return bool(
