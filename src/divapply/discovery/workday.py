...
def setup_proxy(proxy_str: str | None) -> None:
    ...
    if len(parts) == 4:
        ...
        log.warning("Proxy format not recognized (expected host:port:user:pass or host:port)")
        _opener = urllib.request.build_opener()
        return
...
def _process_one(
    ...
    # Filter by excluded titles before fetching details (saves time)
    if title_excludes:
        before = len(jobs)
        jobs = [j for j in jobs if _title_ok(j.get("title"), title_excludes)]
