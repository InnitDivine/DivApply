...
def _job_location_text(value: object) -> str:
...
def _extract_job_posting_schema(soup) -> dict[str, str]:
    ...
    for tag in soup.find_all("script", attrs={"type": "application/ld+json"}):
        ...
        try:
            ...
            continue
        for node in _flatten_json_ld_items(parsed):
            if not _json_ld_type_matches(node, "JobPosting"):
                continue
            ...
            location = _job_location_text(node.get("jobLocation"))
            if location:
...
def _extract_manual_job_metadata(url: str) -> dict[str, str | bool]:
    ...
    schema_job = _extract_job_posting_schema(soup)
