...
- Logs: apply and agent logs can include URLs, form labels, file paths, and snippets. DivApply creates them without following links, enforces user-only permissions, redacts credentials and URL queries, and expires them after 30 days by default. Treat them as private.
- Browser state: dedicated worker profiles, cookies, and sessions are local only. DivApply does not clone a personal Chrome profile; legacy unmarked Chrome worker profiles are preserved but refused.
- Apply-agent staging: resume/cover-letter copies, prompt, and MCP config are isolated per worker and removed together when the run ends.
- Apply-agent browser authority: page JavaScript evaluation and arbitrary browser code are unavailable. An automated `APPLIED` result is accepted only when the agent ends with an origin-bound three-line proof block matching the saved job/application origin. This is a fail-closed model report, not independent browser-event attestation; review application history and employer confirmation.
...
- `divapply backup` excludes logs and secrets by default. Add `--include-logs` or `--include-secrets` only when the recovery need outweighs the extra exposure.
