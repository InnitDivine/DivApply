...
def test_archived_jobs_are_excluded_from_followups_and_analytics(tmp_path) -> None:
    db_path = tmp_path / "divapply.db"
    conn = init_db(db_path)
    url = "https://example.com/archived-followup"
    conn.execute(
        "INSERT INTO jobs (url, title, company, discovered_at) VALUES (?, ?, ?, ?)",
        (url, "Archived Analyst", "Example", "2026-01-01"),
    )
    conn.commit()
    add_application_event(
        url,
    ...
    assert archive_job(url, conn=conn) is True

    assert get_due_followups(today="2026-01-03", conn=conn) == []
    analytics = get_application_analytics(conn)
    assert analytics["states"] == []
    assert analytics["events"] == []
    assert analytics["due_followups"] == 0
    close_connection(db_path)
...
def test_store_jobs_soft_dedups_by_canonical_key(tmp_path) -> None:
    ...
    assert new == 1
    assert existing == 1
    assert canonical_job_key("Support Analyst", "Acme Inc.", "Sampleville, YY") == canonical_job_key(
        "Support Analyst", "Acme", "Sampleville YY"
    )
    close_connection(db_path)


def test_canonical_job_key_requires_title_and_company_to_avoid_sparse_false_matches() -> None:
    assert canonical_job_key("Support Analyst", None, "Sampleville, YY") is None
    assert canonical_job_key(None, "Acme", "Sampleville, YY") is None
...
def test_archive_job_hides_job_from_ready_apply_counts(tmp_path) -> None:
    ...
    for stage in ("discovered", "enriched", "pending_score", "scored", "pending_tailor", "tailored", "pending_cover", "pending_apply", "applied"):
        assert get_jobs_by_stage(conn=conn, stage=stage) == []
    assert get_jobs_by_stage(conn=conn, stage="archived")[0]["url"] == "https://example.com/archive-me"
    close_connection(db_path)


def test_archive_job_refuses_in_progress_application(tmp_path) -> None:
    db_path = tmp_path / "divapply.db"
    conn = init_db(db_path)
    conn.execute(
        "INSERT INTO jobs (url, title, apply_status, discovered_at) VALUES (?, ?, ?, ?)",
        ("https://example.com/in-progress", "In Progress", "in_progress", "2026-01-01"),
    )
    conn.commit()

    assert archive_job("https://example.com/in-progress", conn=conn) is False
    row = conn.execute(
        "SELECT archived_at, apply_status FROM jobs WHERE url = ?",
...
def test_record_reliability_event_is_durable_when_it_owns_the_transaction(tmp_path) -> None:
    db_path = tmp_path / "divapply.db"
    conn = init_db(db_path)

    record_reliability_event("worker_crash", "Worker crashed", conn=conn)
    close_connection(db_path)

    reopened = sqlite3.connect(db_path)
    count = reopened.execute(
        "SELECT COUNT(*) FROM reliability_events WHERE category = ?",
        ("worker_crash",),
    ).fetchone()[0]
    reopened.close()
    assert count == 1
...
def test_record_reliability_event_does_not_commit_callers_transaction(tmp_path) -> None:
    db_path = tmp_path / "divapply.db"
    conn = init_db(db_path)
    conn.execute("BEGIN")

    record_reliability_event("inside_tx", "Inside transaction", conn=conn)
